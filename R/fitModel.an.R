...
#' @examples
#'
fitModel.an <- function(no=1,mo,lo){
  ...
  if(!(no %in% seq(min(CylinderRoots$n), max(CylinderRoots$n)) )){
    stop("Wrong mode number no")}

  # Check ranges
  if((lo>max(CylinderRoots[n==no]$l))){
    warning("lo>max(l) for mode no")
    }


  DATA <- CylinderRoots[n==no] #& between(m,(1-OSF)*mo,(1+OSF)*mo)& between(l,(1-OSF)*lo,(1+OSF)*lo)]
